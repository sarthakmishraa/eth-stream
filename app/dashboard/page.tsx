...
const Dashboard = () => {
  ...
  const [currencyToSearch, setCurrencyToSearch] = useState<
    string | null
  >(null);
  const [currencySearched, setCurrencySearched] =
  ...
  const handleCurrencySearch = () => {
    const curr = data?.find(
      (curr) => curr.id === currencyToSearch
    );
    if (curr) setCurrencySearched(curr);
  };

  const generateGraph = async () => {
    try {
      const coincapAPIKey =
        process.env.NEXT_PUBLIC_COINCAP_API_KEY;
      const URL = `${process.env.NEXT_PUBLIC_API_URL}/${currencySearched?.id}/history?interval=${interval}&apiKey=${coincapAPIKey}`;
    ...
    const getData = async () => {
      try {
        const coincapAPIKey =
          process.env.NEXT_PUBLIC_COINCAP_API_KEY;
        const url =
          `${process.env.NEXT_PUBLIC_API_URL}?apiKey=${coincapAPIKey}` ??
          "";
  ...
  if (!data) {
    return (
      <p className="text-center text-2xl p-20 md:px-40 lg:px-60">
        Loading...
      </p>
    );
  }
  ...
  return (
    <div className="text-lg px-20 md:px-40 lg:px-60">
      <div className="flex flex-col items-center text-center space-y-4">
        <div>
          <p className="text-4xl my-10 leading-snug tracking-tight font-bold">
            Select a cryptocurrency from the dropdown and
            click "Go" to view its real-time data.
          </p>
        ...
        <div className="flex items-center">
          <select
            className="bg-fontColor text-bgColor cursor-pointer border-2 p-1 rounded-md"
            onChange={(e) =>
              setCurrencyToSearch(e.target.value)
            }
          >
        ...
        {currencySearched && (
          <div>
            <div className="text-left space-y-2">
              {currencySearched.name && (
                <p>
                  Name of the currency:{" "}
                  {currencySearched.name}
                </p>
              )}
              {currencySearched.symbol && (
                <p>Symbol: {currencySearched.symbol}</p>
              )}
              {currencySearched.rank && (
                <p>
                  Rank (in ascending order):{" "}
                  {currencySearched.rank}
                </p>
              )}
              {currencySearched.priceUsd && (
                <p>
                  Price: $
                  {parseFloat(
                    currencySearched.priceUsd
              ...
              {currencySearched.symbol &&
                currencySearched.maxSupply && (
                  <p>
                    Total quantity of{" "}
                    {currencySearched.symbol} issued:{" "}
                    {parseFloat(
                      currencySearched.maxSupply
                    ).toFixed(2)}
                  </p>
                )}
              {currencySearched.changePercent24Hr && (
                <p>
                  Value change in the last 24 hrs:{" "}
                  {currencySearched.changePercent24Hr}
                </p>
              )}
              {currencySearched.marketCapUsd && (
                <p>
                  Market Cap: $
                  {parseFloat(
                    currencySearched.marketCapUsd
